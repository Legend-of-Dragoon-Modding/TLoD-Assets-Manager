...
class gltfFile:
    def __init__(self, gltf_to_convert=dict, gltf_deploy_path=str) -> None:
        """
        glTF Converter: Convert the model from human readable\n
    ...
    def write_gltf(self) -> None:
        ...
        file_name = self.gltf_to_convert.get('Name')
        
        # Setup an empty glTF File
        gltf_file = GLTF2()

        # Setup Asset
        gltf_file.asset.generator = f'TLoD Assets Converter ALPHA 0.3'
        ...
        for this_node_number in range(0, total_objects_gltf):
            ...
            gltf_nodes.name =  f'{file_name}_{this_node_number}'
        ...
        for this_object_material in range(0, total_objects_gltf):
            ...
            material_gltf.name = f'{file_name}_{this_object_material}'
        ...
        for this_gltf_mesh in meshes_gltf:
            ...
            gltf_mesh = Mesh()
            gltf_mesh.name = f'{file_name}_{current_mesh_number}'
        ...
        binary_file_name = f'{file_name}_buffers.bin'
        ...
        gltf_file.save_json(f'{self.gltf_deploy_path}\\{file_name}.gltf')
        ...
        with open(f'{self.gltf_deploy_path}\\{file_name}_buffers.bin', 'wb') as gltf_binary_buffer:
