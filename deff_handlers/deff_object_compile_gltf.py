...
DEFF Object Compile glTF: this module will take the DEFF Object Data,
and re-shape it for later being written into glTF/Buffer Files.
I redo this module specially to handle DEFF Objects.
...
class NewGltfDeffObject:
    def __init__(self, data_to_convert=dict, vsync_divider=int) -> None:
        """
        New glTF DEFF:\n
        This is the actual module which will take the converted data\n
        and write it into glTF Format.
        """
        self.data_to_convert = data_to_convert
        self.deff_vsync_divider = vsync_divider
        self.gltf_format: dict = {}
        self.object_to_gltf_dict()
    
    def object_to_gltf_dict(self) -> None:
        """
        Object To glTF Dict:\n
        Take all the DEFF Object Data and create a dict to later write the glTF File and Buffer.
        """
        #First i need to get the important properties and data
        model_name = self.data_to_convert.get('Name')
        denest_model_data = self.data_to_convert.get('Model')
        model_data = denest_model_data.get('Converted_Data')
        animation_data = self.data_to_convert.get('Animations')
        frame_start = self.data_to_convert.get('Frame Start')
        frame_end = self.data_to_convert.get('Frame End')

        #I will do some calcs prior using them, just to avoid repetition
        animation_total_frames = (frame_end // self.deff_vsync_divider) - (frame_start // self.deff_vsync_divider)

        """
        Changing the Generic 'Object_Number_n' to 'NameOfModel_Obj_n', this is good to avoid name
        shadowing in Blender Importing, since all objects will be in a single Hierarchy and much of the Objects get repeated.
        """
        model_dict: dict = {}
        current_object_index = 0
        for current_object_name in model_data:
            get_object_data = model_data.get(f'{current_object_name}')
            new_object_name: str = ''
            if model_name not in current_object_name:
                new_object_name = f'{model_name}_Obj_{current_object_index}'
            else:
                new_object_name = current_object_name
            this_object_new = {f'{new_object_name}': get_object_data}
            model_dict.update(this_object_new)
            current_object_index += 1

        #Take the New conformed Data and re-shape it into glTF Dict
        conformed_gltf = self.generate_gltf_data(model_name=model_name, model_dict=model_dict, animation_dict=animation_data, total_frames=animation_total_frames)
    
    def generate_gltf_data(self, model_name=str, model_dict=dict, animation_dict=dict, total_frames=int) -> None:
        """
        Generate glTF Data: \n
        ...
        buffer_size = 0
        for current_object in model_dict:
            get_this_object = model_dict.get(f'{current_object}')
        ...
        # TODO: Need to rewrite all this thing
        this_animation_index = this_data_index
        this_animation_buffer_size = buffer_size
        gltf_animation: dict = {}
        total_anim_buffer_size: list = []

        for current_object_animation in animation_dict:
            current_animation_data = animation_dict.get(f'{current_object_animation}')
            this_anim_total_transforms = current_animation_data.get(f'TotalTransforms')
            this_anim_object_count = current_animation_data.get(f'ObjectCount')
            this_anim_animation_type = current_animation_data.get(f'AnimationType')
            this_anim_data = current_animation_data.get(f'AnimationsData')
            ...
            gltf_generate_links = self.generate_animation_link(objs_nums=this_anim_object_count, anim_name=current_object_animation, last_accessor_num=this_animation_index)
            ...
            this_animation = {f'{current_object_animation}': {'AnimLinks': gltf_animation_link, 'AnimAccessors': gltf_generate_anim_accessors, 
                                                                'AnimBuffersView': gltf_generate_anim_bufferviews, 'AnimBuffers': gltf_animation_data}}
            
            gltf_animation.update(this_animation)
            total_anim_buffer_size.append(get_size_info)
            this_animation_index += next_animation_index
            this_animation_buffer_size += next_anim_bufferviews_size

        total_gltf_bin_size = sum(total_anim_buffer_size) + buffer_size
        current_model_gltf = {'ObjectsNumber': gltf_total_objects, 'Meshes': gltf_meshes_data, 'Buffers': gltf_to_binary_data, 
                                ...
                                'Animations': gltf_animation, 'Name': model_name}
        
        self.gltf_format.update(current_model_gltf)
    ...
    def animation_to_gltf_buffer(self, animation_data=dict, total_transforms=int, total_frames=int) -> dict:
        ...
        frames_per_second_calc = 60 / self.deff_vsync_divider # This calc is using var[25] from each DEFF and set it into the General DEFF Properties
        keyframe_factor = (total_frames / frames_per_second_calc) / total_transforms # This value would change related to the total frames vs frames vs transforms
        current_time = 0.0
        ...
        animation_info = {'Keyframes': total_transforms, 'Min': 0.0, 'Max': current_time, 'BufferSizes': anim_buffer_sizes,
                          'BufferSizeSum': total_anim_buffer_size} # Original 'Max': (keyframe_factor - 0.2) This is ok?
